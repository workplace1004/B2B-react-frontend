    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[24px] font-bold text-gray-900 dark:text-white">Purchasing</h1>
          </div>
          {(!filteredPOs || filteredPOs.length === 0) ? null : (
        ...
        {filteredPOs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
              Get started by adding your first purchase order to the inventory.
            </p>
            <button className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
              <Plus className="w-5 h-5" />
              New Purchase Order
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {filteredPOs.map((po: any) => (
                <tr key={po.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-white">{po.supplier}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-500 dark:text-white">{po.amount}</td>
