...
export default function QuotesProformas() {
  ...
  const orders = data || [];
  const quotes = orders.filter((order: any) => order.status === 'DRAFT' || order.status === 'PENDING');
        ...
        {quotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
              Get started by creating your first quote or proforma invoice.
            </p>
            <button className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
              <Plus className="w-5 h-5" />
              New Quote
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {quotes.map((quote: any) => (
                <tr key={quote.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-white">{quote.customer?.name || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-white">${parseFloat(quote.totalAmount || 0).toFixed(2)}</td>
