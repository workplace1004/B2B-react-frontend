...
export default function SizeFit() {
  ...
  const products = data || [];
  const uniqueSizes: string[] = Array.from(new Set(products.flatMap((p: any) => p.sizes || []))) as string[];
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          ...
          {(!uniqueSizes || uniqueSizes.length === 0) ? null : (
        ...
        {uniqueSizes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
              Get started by adding your first size chart to the inventory.
            </p>
            <button className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
              <Plus className="w-5 h-5" />
              Add Size Chart
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {uniqueSizes.map((size: string) => {
                const productsWithSize = products.filter((p: any) => p.sizes?.includes(size));
                return (
                  <tr key={size} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      {size}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-white">{productsWithSize.length}</td>
