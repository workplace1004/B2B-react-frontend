...
export default function ProductionMRP() {
  const { data, isLoading } = useQuery({
    queryKey: ['purchase-orders', 'production'],
    queryFn: async () => {
      try {
        // Using orders as placeholder for production orders
  ...
  return (
    <div>
      <Breadcrumb currentPage="Production (MRP)" />
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[24px] font-bold text-gray-900 dark:text-white">Production (MRP)</h1>
          </div>
          {(!productionOrders || productionOrders.length === 0) ? null : (
        ...
        {productionOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
              Vendors & Factories, Production Orders, Landed Cost
            </p>
            <button className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">
              <Plus className="w-5 h-5" />
              New Production Order
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Vendor</th>
